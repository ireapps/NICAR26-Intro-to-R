...
This first code chunk makes sure the "working directory" of the file is the base folder, where the .Rproj file lives:
...
To practice joins, we'll use two tables of information about WNBA players during the 2024 season. One table (*wnba_salaries_2024.csv*) comes from [Her Hoop Stats](https://herhoopstats.com/) and the other (*wnba_teams_2024.csv*) comes from [ESPN](https://www.espn.com/wnba/).
...
wnba_salaries %>% 
  inner_join(wnba_teams, by = c("name"="player_name")) %>% 
  group_by(team) %>% 
  summarise(avg_sal = mean(salary_2024, na.rm=T)) %>% 
